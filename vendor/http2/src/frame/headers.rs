...
#[derive(Debug)]
enum FieldIter {
    Map(header::IntoIter<HeaderValue>),
...
struct HeaderBlock {
    ...
    /// Exact ordinary-header order for an outgoing request.
...
impl Headers {
    ...
    pub(crate) fn set_ordered_fields(&mut self, fields: Vec<(HeaderName, HeaderValue)>) {
        self.header_block.ordered_fields = Some(fields);
    }
...
impl Iterator for FieldIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::Map(fields) => fields.next(),
            Self::Ordered(fields) => fields.next().map(|(name, value)| (Some(name), value)),
        }
    }
}
...
impl HeaderBlock {
    ...
    fn into_encoding(self, encoder: &mut hpack::Encoder) -> EncodingHeaderBlock {
        ...
        let fields = match self.ordered_fields {
            Some(fields) => FieldIter::Ordered(fields.into_iter()),
            None => FieldIter::Map(self.fields.into_iter()),
        };
        let headers = Iter {
