...
reports forbidden peer Close codes instead of rewriting them. A small final
patch exposes ordered negotiation for Phantom's custom handshake, rejects
terminal raw-DEFLATE streams, and keeps an 8-bit negotiated window within its
wire bound. The `deflate` feature enables only its codec and HTTP value types;
the separate
