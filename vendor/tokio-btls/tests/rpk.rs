...
// BoringSSL implements RFC 7250 handshakes. Our rpk.patch restores the
// two-step credential APIs used by btls, so exercise those shims end to end.
// https://datatracker.ietf.org/doc/html/rfc7250

use btls::pkey::PKey;
use btls::ssl::{
    CertificateType, Error as SslError, Ssl, SslAcceptor, SslAlert, SslConnector, SslCredential,
    SslMethod, SslVerifyError, SslVerifyMode,
};
use futures::future;
...
fn create_server() -> (
    impl Future<Output = Result<SslStream<TcpStream>, SslError>>,
    SocketAddr,
) {
