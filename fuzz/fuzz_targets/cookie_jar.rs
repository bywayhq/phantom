...
//!
//! The target drives `phantom::CookieJar::set_cookie` and
//! `phantom::CookieJar::request_value` in
//! `crates/phantom/src/session/cookies.rs`. That is the jar a `Client` uses
//! for every response `Set-Cookie` field and every request `Cookie` field, so
//! the fuzzed bytes take the same path a hostile origin's `Set-Cookie` takes.
//!
...
fn exercise(fields: &[String]) -> bool {
...
fuzz_target!(|input: &[u8]| {
    let _ = exercise(&fields(input));

    let structured = perturb(VALID_SET_COOKIES, input);
    let stored = exercise(&fields(&structured));
    if input.is_empty() {
        assert!(stored, "structural seeds must remain storable cookies");
    }
});
