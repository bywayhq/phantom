...
| Proxy credentials | Literal `Proxy-Authorization` fields are rejected. With Basic credentials on the proxy, the first connection to that proxy starts anonymously and replays once, on a fresh connection over the same route, only after a strict `407` Basic challenge. Once the proxy accepts the credentials, later tunnels to it send them on the first CONNECT. |
