...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder plus finite client-owned reconnects with `Last-Event-ID`, server retry delay, optional cancellation-safe DATA-activity idle timeout, cookies, and 204 termination; feature-gated WS/H1 over its direct and proxy routes plus exact direct WS/H2 extended CONNECT for explicit custom profiles; ordered customizable handshakes, challenge-driven forward-proxy Basic authentication, typed opt-in `permessage-deflate`, client cookies, bounded messages, `Stream`/`Sink`, and protocol-specific strict response validation | Caller-positionable `Last-Event-ID`, an optional Firefox-style minimum retry delay, and immediate reconnect after pre-response network errors, from retained Chrome 153 and Firefox 155 Windows HTTP/1.1 captures (no browser jitter was observed); H2/H3, macOS, and Safari SSE behavior is not yet captured; other WebSocket extensions, named compression/send policies, automatic reconnect, named-browser/proxy H2 extended CONNECT, and H3 only from evidence |
...
Phantom currently exposes response bodies exactly as received. It does not
decode HTTP `Content-Encoding`, and it does not insert `Accept-Encoding` into
the caller's ordered request fields. A caller that advertises `gzip`,
`deflate`, `br`, or `zstd` currently owns decoding the returned body. Opt-in
streaming response decompression is Phase 1 work; automatic request-field
insertion is not, because its captured wire position belongs to the profile or
caller.
