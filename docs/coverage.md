...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder plus finite client-owned reconnects with `Last-Event-ID`, server retry delay, optional cancellation-safe DATA-activity idle timeout, cookies, and 204 termination; feature-gated WS/H1 with direct plaintext, plaintext over HTTP forward proxies, and direct/HTTP(S)-CONNECT/SOCKS5 TLS routes, ordered customizable handshake, challenge-driven forward-proxy Basic authentication, typed opt-in `permessage-deflate`, client cookies, bounded messages, `Stream`/`Sink`, and strict 101 validation | SSE jitter and broader browser reconnect differentials; plaintext SOCKS5 WebSocket, other WebSocket extensions, named compression/send policies, automatic reconnect, H2 extended CONNECT, and H3 only from evidence |
