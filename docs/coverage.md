...
| HTTP/1.1 | Ordered request fields and static trailers; ordinary methods with owned-byte or pull-driven streaming bodies; exact content-length validation; response field order, duplicate interleaving, and name spelling; streaming/backpressured responses; client-owned sequential keep-alive reuse with bounded waiters and no pipelining; finite opt-in HTTPS redirects; direct HTTPS and plaintext HTTP; unauthenticated absolute-form forwarding for `http://` origins over plaintext or TLS proxies with independent proxy authentication policy; HTTP/HTTPS CONNECT and local-/remote-DNS SOCKS5 routes; byte-preserving Upgrade handoff | Streaming-body-produced trailers, parallel connection policy, retries, authenticated forwarding, and additional proxy modes |
| HTTP/2 | Ordered SETTINGS, request and response fields, static request trailers, window update, pseudo-header order, priority, ALPS peer settings and connection-scoped `ACCEPT_CH`, flow-controlled owned or pull-driven request bodies and streaming responses; client-owned exact-origin/route reuse with bounded local active work and waiters, peer stream-limit enforcement, stream-scoped cancellation, and one replacement-connection retry for a bodyless GET rejected by `GOAWAY(NO_ERROR)` | Streaming-body-produced trailers, configurable general retry policy, and extended CONNECT where evidence requires it |
