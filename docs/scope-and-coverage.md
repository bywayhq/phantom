...
| HTTP/3 | Forced direct one-shot request; exact `h3` ALPN; typed ordered SETTINGS and request fields; Chrome nonzero inbound QPACK, randomized GREASE, and H3 DATAGRAM policy; bounded dynamic response decode; capture-matching outbound dynamic QPACK codec; streaming data/trailers; body cancellation and bounded shutdown | Live connection-owned outbound QPACK delivery, multiplexed sessions, extension-specific datagram APIs, qlog; UDP proxies later |
