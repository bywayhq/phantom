...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder plus finite session-owned reconnects with `Last-Event-ID`, server retry delay, optional cancellation-safe DATA-activity idle timeout, cookies, and 204 termination; feature-gated WSS/H1 with ordered customizable handshake, typed opt-in `permessage-deflate`, direct/CONNECT/SOCKS5 routes, session cookies, bounded messages, `Stream`/`Sink`, and strict 101 validation | SSE jitter and broader browser reconnect differentials; other WebSocket extensions, named compression/send policies, automatic reconnect, H2 extended CONNECT, and H3 only from evidence |
| Routes | Direct TCP/UDP paths; plaintext HTTP CONNECT and credential-capable RFC 1929 local-/remote-DNS SOCKS5 for H1/H2 origin TLS and H1 WSS, with canonical Unicode proxy and origin hosts, typed negotiation, and no direct fallback; H1/H2 pool identity includes the complete route, DNS mode, and credentials; H3 rejects both TCP-only routes before I/O | HTTP forwarding, HTTPS proxy and its authentication challenges, custom SOCKS5 resolvers, SOCKS5 UDP ASSOCIATE, then CONNECT-UDP/MASQUE |
