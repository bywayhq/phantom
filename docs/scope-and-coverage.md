...
| HTTP/1.1 | Ordered request fields; response field order, duplicate interleaving, and name spelling; streaming/backpressured responses; session-owned sequential keep-alive reuse with bounded waiters and no pipelining; direct, plaintext HTTP CONNECT, and remote-DNS SOCKS5 routes; byte-preserving Upgrade handoff | Parallel connection policy, redirects/retries, forwarding and additional proxy modes |
