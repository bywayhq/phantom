...
| HTTP/1.1 | Ordered request fields; ordinary methods with owned-byte or pull-driven streaming bodies; exact content-length validation; response field order, duplicate interleaving, and name spelling; streaming/backpressured responses; client-owned sequential keep-alive reuse with bounded waiters and no pipelining; finite opt-in HTTPS redirects; direct HTTPS and plaintext HTTP, plaintext HTTP forwarding, HTTP/HTTPS CONNECT, and local-/remote-DNS SOCKS5 routes; byte-preserving Upgrade handoff | Request trailers, parallel connection policy, retries, authenticated/TLS forwarding and additional proxy modes |
