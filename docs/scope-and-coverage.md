...
| HTTP/1.1 | Ordered request fields; response field order, duplicate interleaving, and name spelling; streaming/backpressured responses; cancellation tests; direct and plaintext HTTP CONNECT routes; byte-preserving Upgrade handoff | Pooling, redirects/retries, forwarding and additional proxy schemes |
...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder with WHATWG field semantics; feature-gated WSS/H1 with ordered customizable handshake, direct/CONNECT routes, session cookies, bounded messages, `Stream`/`Sink`, and strict 101 validation | SSE reconnect/session policy; WebSocket compression/extensions, automatic reconnect, H2 extended CONNECT, and H3 only from evidence |
