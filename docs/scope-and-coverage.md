...
| QUIC | BoringSSL-backed Quinn client handshake, packet/header protection, key updates, Retry and endpoint HMAC; typed captured transport settings; seeded exact transport-parameter serializer with randomized permitted order and GREASE; bounded opt-in NSS key-log queue; payload-free QUIC v1 packet analyzer; controlled Chrome/Phantom decrypted captures through the first request | Reusable connection lifecycle, stable logical-flight comparator, packet-shape stability study |
| HTTP/3 | Forced direct one-shot request; exact `h3` ALPN; typed ordered SETTINGS and request fields; Chrome nonzero inbound QPACK, randomized GREASE, and H3 DATAGRAM policy; bounded dynamic response decode and connection-owned request encode; live capture-matching encoder-stream and HEADERS bytes; streaming data/trailers; body cancellation and bounded shutdown; single-connection bounded qlog | Multiplexed sessions, fresh-browser packet-level request differential, extension-specific datagram APIs; UDP proxies later |
...
| Validation | Local TLS/H2/QUIC fixtures, hostile H1/H2 peers, bounded qlog/key-log seams, deterministic authenticated QUIC packet analysis, controlled Chrome/Phantom packet evidence, Peet/Pingly as supplemental observers | Repeated packet-shape study, Prism comparison, fuzzing, sanitizers, long soaks |
