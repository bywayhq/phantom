...
| Public client | Immutable exact-H1/H2 facade; additive DER trust roots; default and per-request routes; ordered origin/CONNECT fields; unified streaming response body; typed stable error categories | H3 facade after typed QUIC construction, then pooling and session state |
...
| Routes | Direct TCP/UDP paths; plaintext HTTP CONNECT for H1/H2 origin TLS with bounded negotiation and no direct fallback | HTTP forwarding, HTTPS proxy, SOCKS5 local/remote DNS, UDP ASSOCIATE, then CONNECT-UDP/MASQUE |
| Validation | Local TLS/H2/QUIC fixtures, hostile H1/H2/H3 peers, bounded qlog/key-log seams, deterministic authenticated QUIC packet analysis, controlled Chrome/Phantom packet evidence, Peet/Pingly as supplemental observers | Repeated packet-shape study, Prism comparison, fuzzing, sanitizers, long soaks |
