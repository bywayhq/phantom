...
| Session policy | Cloneable isolated session; bounded H2 retained pool; bounded direct-H3 retained pool with local stream admission; optional bounded cookie jar with deterministic path/creation order, PSL/prefix/expiry checks, and explicit activation | H1 reuse, redirects, retries, SameSite request context, CHIPS, client hints, tickets, DNS/Alt-Svc state |
