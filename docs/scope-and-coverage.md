...
| Session policy | Cloneable isolated session; bounded H2 and direct-H3 retained pools with per-origin-and-route active and waiting admission; optional bounded cookie jar with deterministic path/creation order, PSL/prefix/expiry checks, and explicit activation | H1 reuse, redirects, retries, SameSite request context, CHIPS, client hints, tickets, DNS/Alt-Svc state |
