...
| QUIC | BoringSSL-backed Quinn client handshake, packet/header protection, key updates, Retry and endpoint HMAC; typed captured transport settings; seeded exact transport-parameter serializer with randomized permitted order and GREASE | Reusable connection lifecycle, decrypted packet differential |
| HTTP/3 | Forced direct one-shot request; exact `h3` ALPN; typed ordered SETTINGS and request fields; Chrome nonzero inbound QPACK, randomized GREASE, and H3 DATAGRAM policy; bounded dynamic response decode and connection-owned request encode; live capture-matching encoder-stream and HEADERS bytes; streaming data/trailers; body cancellation and bounded shutdown | Multiplexed sessions, packet-level request differential, extension-specific datagram APIs, qlog; UDP proxies later |
