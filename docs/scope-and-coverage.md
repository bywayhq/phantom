...
| HTTP/1.1 | Ordered request fields; ordinary methods with optional owned byte bodies; exact content-length validation; response field order, duplicate interleaving, and name spelling; streaming/backpressured responses; session-owned sequential keep-alive reuse with bounded waiters and no pipelining; finite opt-in redirects; direct, HTTP/HTTPS CONNECT, and local-/remote-DNS SOCKS5 routes; byte-preserving Upgrade handoff | Streaming request bodies, parallel connection policy, retries, forwarding and additional proxy modes |
...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder plus finite session-owned reconnects with `Last-Event-ID`, server retry delay, optional cancellation-safe DATA-activity idle timeout, cookies, and 204 termination; feature-gated WSS/H1 with ordered customizable handshake, typed opt-in `permessage-deflate`, direct/HTTP(S)-CONNECT/SOCKS5 routes, session cookies, bounded messages, `Stream`/`Sink`, and strict 101 validation | SSE jitter and broader browser reconnect differentials; other WebSocket extensions, named compression/send policies, automatic reconnect, H2 extended CONNECT, and H3 only from evidence |
| Routes | Direct TCP/UDP paths; HTTP/1.1 CONNECT over plaintext or independently authenticated TLS proxies, plus credential-capable RFC 1929 local-/remote-DNS SOCKS5 for H1/H2 origin TLS and H1 WSS, with canonical Unicode proxy and origin hosts, typed negotiation, and no direct fallback; HTTPS proxy and origin trust and ticket caches are isolated; H1/H2 pool identity includes the complete route, DNS mode, and credentials; H3 rejects all TCP-only routes before I/O | HTTP forwarding, proxy authentication challenges, H2 proxy transport, custom SOCKS5 resolvers, SOCKS5 UDP ASSOCIATE, then CONNECT-UDP/MASQUE |
