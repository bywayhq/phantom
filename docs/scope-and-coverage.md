...
| QUIC | BoringSSL-backed Quinn client handshake, packet/header protection, key updates, Retry and endpoint HMAC; typed captured transport settings; seeded exact transport-parameter serializer with randomized permitted order and GREASE; bounded opt-in NSS key-log queue; payload-free QUIC v1 packet analyzer | Reusable connection lifecycle, fresh-browser decrypted packet differential |
...
| Validation | Local TLS/H2/QUIC fixtures, hostile H1/H2 peers, bounded qlog/key-log seams, deterministic authenticated QUIC packet analysis, Peet/Pingly as supplemental observers | Fresh-browser decrypted packet differential, Prism comparison, fuzzing, sanitizers, long soaks |
