...
| HTTP/1.1 | Ordered request fields, streaming/backpressured responses, cancellation tests, direct and plaintext HTTP CONNECT routes | Pooling, redirects/retries, forwarding and additional proxy schemes, WebSocket Upgrade |
| HTTP/2 | Ordered SETTINGS, window update, pseudo/header order, priority, ALPS peer settings, streaming flow control; cloneable reusable connection with sequential/concurrent streams and stream-scoped cancellation | Public-client pooling, GOAWAY admission and draining, bounded waiters, richer request bodies, extended CONNECT where evidence requires it |
...
| Higher protocols | Ordinary streaming response body; feature-gated bounded SSE decoder with WHATWG field semantics | SSE reconnect/session policy and H1 WebSocket; later extended CONNECT/H3 only from evidence |
| Routes | Direct TCP/UDP paths; plaintext HTTP CONNECT for H1/H2 origin TLS with bounded negotiation and no direct fallback; H3 rejects that TCP-only route before I/O | HTTP forwarding, HTTPS proxy, SOCKS5 local/remote DNS, UDP ASSOCIATE, then CONNECT-UDP/MASQUE |
