...
|---|---|---|---|---|---|---|---|
| **Phantom current** | Immutable routed client plus stateful session and H1/H2/H3 pools | Exact H1/H2/H3 ordinary requests with optional owned bodies; ordered origin, forwarded absolute-form, and CONNECT fields; streaming response body | Public typed TLS/H2/H3/QUIC profiles; H3 TLS is distinct | Additive origin and independent HTTPS-proxy trust roots; direct TCP/UDP, plaintext H1 forwarding, HTTP/HTTPS CONNECT, or local-/remote-DNS SOCKS5 for H1/H2; session cookies and bounded secure-origin client-hint negotiation; no learned DNS yet | Response body, bounded SSE decoder with finite session reconnects, and exclusive H1 WebSocket over all TCP routes; no H2 extended CONNECT or H3 WebSocket policy | Stable facade/proxy/SSE/WebSocket error kinds; completion shuts down; early body drop cancels/resets; unsupported route/protocol pairings fail pre-I/O | No backend escape; unsupported surfaces absent |
...
- Resumption controls until session-owned ticket, PSK, partitioning, and replay
...
| **Current facade** | `Client::builder(profile).route(route).add_root_certificate_der(...).add_proxy_root_certificate_der(...).build()` | Exactly one required immutable profile and a default direct, HTTP forwarding/CONNECT, or SOCKS5 route; origin and HTTPS-proxy roots and certificate policy are independent. No backend callbacks or pool builder. |
| **Current facade** | `Client::get(HttpProtocol::{Http1,Http2,Http3}, uri) -> Result<RequestBuilder, RequestError>` | Exact one-shot routed HTTPS plus plaintext HTTP through exact-H1 forwarding; ordered origin, absolute-form, and CONNECT fields; URI-owned authority; request-scoped route override; unsupported pairings and malformed requests fail before network I/O. H3 is direct-only and rejects TCP-only proxy routes pre-I/O. |
...
| **Current session slice** | `Client::session() -> Session`; `SessionBuilder` configures bounded H1/H2/H3 retention and admission, optional bounded cookies, bounded client-hint origins, and finite redirects | Session clones share exact-origin/route pools, scoped TLS tickets, redirect policy, any enabled jar, and response `Accept-CH` state; separately created sessions share none of them. Selected H2/H3 connections apply exact-origin ALPS `ACCEPT_CH` without persisting it. `Critical-CH` and one bodyless H2 GET rejected by `GOAWAY(NO_ERROR)` each have one narrow replay path. General retry policy, DNS/HTTPS answers, and Alt-Svc remain absent. |
