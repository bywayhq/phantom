...
| **Current facade** | `Client::builder(profile).route(route).add_root_certificate_der(...).add_proxy_root_certificate_der(...).build()` | Exactly one required immutable profile and a default direct, HTTP(S)-CONNECT, or SOCKS5 route; origin and HTTPS-proxy roots and certificate policy are independent. No backend callbacks or pool builder. |
| **Current facade** | `Client::get(HttpProtocol::{Http1,Http2,Http3}, uri) -> Result<RequestBuilder, RequestError>` | Exact one-shot routed HTTPS; ordered origin and CONNECT fields; URI-owned authority; request-scoped route override; unsupported protocols and malformed requests fail before network I/O. H3 is direct-only and rejects TCP-only proxy routes pre-I/O. |
