...
| **Current session slice** | `Client::session() -> Session`; `SessionBuilder` configures retained H2 capacity and optional bounded cookies | Session clones share exact-origin/route H2 reuse and any enabled jar; separately created sessions share neither. Tickets, DNS/HTTPS answers, Alt-Svc, client hints, redirects, and retries remain absent. |
