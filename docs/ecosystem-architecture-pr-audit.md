...
|---|---|---|---|---|---|---|---|
| **Phantom current** | Immutable routed client plus stateful session and H1/H2/H3 pools | Exact H1/H2/H3 ordinary requests with optional owned bodies; ordered origin and CONNECT fields; streaming response body | Public typed TLS/H2/H3/QUIC profiles; H3 TLS is distinct and caller-supplied pending a retained browser fixture | Additive trust roots; direct TCP/UDP, plaintext HTTP CONNECT, or local-/remote-DNS SOCKS5 for H1/H2; session cookies; no learned DNS/client hints yet | Response body, bounded SSE decoder with finite session reconnects, and exclusive H1 WebSocket over all TCP routes; no H2 extended CONNECT or H3 WebSocket policy | Stable facade/proxy/SSE/WebSocket error kinds; completion shuts down; early body drop cancels/resets; unsupported route/protocol pairings fail pre-I/O | No backend escape; unsupported surfaces absent |
...
| **Current session slice** | `Client::session() -> Session`; `SessionBuilder` configures bounded H1/H2/H3 retention and admission, optional bounded cookies, and finite redirects | Session clones share exact-origin/route pools, scoped TLS tickets, redirect policy, and any enabled jar; separately created sessions share none of them. One bodyless H2 GET rejected by `GOAWAY(NO_ERROR)` is retried on a replacement. General retry policy, DNS/HTTPS answers, Alt-Svc, and client hints remain absent. |
