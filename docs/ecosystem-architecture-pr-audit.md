...
This covers every named reference to the extent evidence was available; its PR
evidence is representative, not exhaustive. BoringSSL, Quinn, and H3 provider
work has separate source audits in this repository. Chromium networking,
NSS/Neqo, Hyper/Reqwest, curl/curl-impersonate, and their full PR histories were
outside this pass.
...
- The authenticated `playground` and Prism repositories were private during
  this pass, so their links are not independently reproducible without access.
  The inspected `playground` branch commit was not associated with a PR. Its
  branch source and decision records, not an inferred review decision, are the
  evidence.
...
Observed: the `phantom` facade now exposes an immutable `Client`, exact H1/H2/H3
selection, ordered request fields, streaming response bodies, independent
origin and HTTPS-proxy trust roots, and owned direct, HTTP/HTTPS-CONNECT, and
local- or remote-DNS-SOCKS5 routes.
The route can be set on the client or overridden by one request; invalid origin
or CONNECT fields fail before proxy I/O, and proxy failure never becomes a direct retry
([facade source](../crates/phantom/src/lib.rs),
[route](../crates/phantom/src/route.rs)). The lower-level request surface is in
`phantom-net`: public HTTP/1, HTTP/2, and HTTP/3 modules expose one-shot
empty-body `send_get` functions, ordered `Vec<RequestHeader>`, parsed `OriginForm`,
...
[HTTP/3](../crates/phantom-net/src/http3/mod.rs),
[request primitives](../crates/phantom-net/src/request.rs)). The profile crate
publicly exposes concrete ordered TLS, HTTP/2, QUIC, and HTTP/3 settings. The
facade's direct H3 path owns an atomic protocol bundle with separate H3 TLS and
rejects the TCP-only CONNECT route before I/O
...
must not silently escape to a direct route. `Session`, SSE, WebSocket, and
additional TCP/UDP route variants remain planned
...
|---|---|---|---|---|---|---|---|
| **Phantom current** | Cheap-clone routed client with H1/H2/H3 pools and bounded cross-request state | Exact H1/H2/H3 ordinary requests with optional owned bodies; ordered origin, forwarded absolute-form, and CONNECT fields; streaming response body | Public typed TLS/H2/H3/QUIC profiles; H3 TLS is distinct | Additive origin and independent HTTPS-proxy trust roots; direct TCP/UDP, plaintext H1 forwarding, HTTP/HTTPS CONNECT, or local-/remote-DNS SOCKS5 for H1/H2; client cookies and bounded secure-origin client-hint negotiation; no learned DNS yet | Response body, bounded SSE decoder with finite client-owned reconnects, and exclusive H1 WebSocket over all TCP routes; no H2 extended CONNECT or H3 WebSocket policy | Stable facade/proxy/SSE/WebSocket error kinds; completion shuts down; early body drop cancels/resets; unsupported route/protocol pairings fail pre-I/O | No backend escape; unsupported surfaces absent |
...
| **obscura** | `Browser::builder`; `Page`; internal transport choice | page navigation/interception; bounded buffered net result | ordinary or Wreq stealth H1/H2 | browser storage/cookies/proxy/SSRF resolver | no page-network WS/SSE; non-network compatibility shims only; bounded download handles proposed | async browser errors; page/isolate lifecycle; tracing | Browser options, not low-level TLS; stealth is feature gated |
...
| playground | No PR was associated with the inspected branch commit. The checked-in decision says ML-DSA advertisement belongs in a narrow `btls-sys` patch, not runtime mutation ([decision](https://github.com/0xARYA/playground/blob/b56c7556ab6a52ff2da5d71eba82a7215e5d29e3/docs/decisions/0159-the-ml-dsa-sigalgs-gap-is-closed-by-patching-btls-sys-not-by.md), [patch](https://github.com/0xARYA/playground/blob/b56c7556ab6a52ff2da5d71eba82a7215e5d29e3/vendor/patches/btls-sys-0001-mldsa-tls-sigalgs.patch)). | A vendor patch can close an otherwise unreachable wire gap, but it needs its own regression vector and vendor check; do not expose a generic signature-algorithm escape hatch. |
...
| Prism | No PRs existed in the authenticated private repository during this pass. | Use its source/tests as observer evidence; do not invent governance conclusions. |
...
|---|---|---|
| **Profile** | Keep concrete, ordered, versioned TLS/H1/H2/H3 recipe types. Built-ins require retained capture evidence; every supported field requires differential coverage, while caller-authored values use the same validation. | uTLS and Wreq show the value of ordered concrete recipes; tls-client #229/#265 and uTLS #333 show why captures and compatibility matter. This reinforces, rather than expands, [configuration.md](configuration.md). |
...
- ECH and trust-anchor wire offers as public customization. Ship captured static
  offer shapes first as versioned profile data; keep ECH discovery and HTTPS
  answers in route/session state.
- Resumption controls until client-owned ticket, PSK, partitioning, and replay
  policy are implemented. Tickets are mutable state, not profile data.
...
|---|---|---|
| **Current** | `phantom::profile::{ClientProfile, Http3ClientSettings, TlsSettings, Http2Settings, ...}` | Owned TCP TLS, optional H2, and optional atomic H3 components; identity metadata remains descriptive and does not switch transport code. |
...
| **Current** | `phantom_profile::{TlsSettings, Http2Settings, Http3Settings, QuicTransportSettings, ...}` | Concrete public TLS/H2/H3/QUIC settings. Chrome QUIC and H3 components are capture-backed; no built-in browser H3 TLS recipe is claimed without a retained ClientHello. |
| **Current facade** | `Client::builder(profile).route(route).add_root_certificate_der(...).add_proxy_root_certificate_der(...).build()` | Exactly one required immutable profile plus client-owned bounded pools, admission, redirects, optional cookies, learned client hints, and a default direct, HTTP forwarding/CONNECT, or SOCKS5 route. Origin and HTTPS-proxy roots and certificate policy are independent. No backend callbacks. |
| **Current facade** | `Client::get(HttpProtocol::{Http1,Http2,Http3}, uri) -> Result<RequestBuilder, RequestError>` | Exact pooled routed HTTPS plus plaintext HTTP through exact-H1 forwarding; ordered origin, absolute-form, and CONNECT fields; URI-owned authority; request-scoped route override; unsupported pairings and malformed requests fail before network I/O. H3 is direct-only and rejects TCP-only proxy routes pre-I/O. |
| **Current facade** | `send() -> Result<Response<ResponseBody>, RequestError>`; `ResponseBody: http_body::Body<Data = Bytes>` | H1/H2/H3 remain pull-driven and preserve their protocol-specific drop/cancel behavior; the facade does not default-buffer. |
| **Current client state** | `ClientBuilder` configures bounded H1/H2/H3 retention and admission, optional bounded cookies, bounded client-hint origins, and finite redirects | Client clones share exact-origin/route pools, authenticated scoped TLS tickets, redirect policy, any enabled jar, and response `Accept-CH` state; independently built clients share none of them. Selected H2/H3 connections apply exact-origin ALPS `ACCEPT_CH` without persisting it. `Critical-CH` and one bodyless H2 GET rejected by `GOAWAY(NO_ERROR)` each have one narrow replay path. General retry policy, DNS/HTTPS answers, and Alt-Svc remain absent. |
| **Planned policy vocabulary** | Extend the closed route vocabulary only when forwarding authentication/proxy TLS, authentication schemes beyond Basic, learned proxy challenge state, custom DNS, and UDP carriage are implemented | Discovery, origin ALPN negotiation, Alt-Svc/HTTPS knowledge, H3 racing, and fallback require separate named policy with a complete contract before they become public. A policy never changes route or substitutes an unprofiled protocol. Do not publish MASQUE, custom DNS, local bind, or H3 route variants before they are applied and tested. |
...
and protocol-specific builders. Graceful client/pool shutdown is also absent
until connection sharing, clone, isolation, and drain ownership are defined.
Each addition needs a concrete use case, an owner, and observable behavioral
tests. Wire-affecting behavior additionally requires retained packet evidence.
