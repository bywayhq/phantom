...
|---|---|---|---|---|---|---|---|
| **Phantom current** | Immutable routed client plus stateful session and H2 pool | Exact H1/H2/H3 GET; ordered origin and CONNECT fields; streaming body | Public typed TLS/H2/H3/QUIC profiles; H3 TLS is distinct and caller-supplied pending a retained browser fixture | Additive trust roots; direct TCP/UDP or plaintext HTTP CONNECT for H1/H2; session cookies; no public DNS/client hints yet | Response body, bounded SSE decoder, and exclusive H1 WebSocket; no upload, reconnect, H2 extended CONNECT, or H3 WebSocket policy | Stable facade/proxy/SSE/WebSocket error kinds; completion shuts down; early body drop cancels/resets; unsupported route/protocol pairings fail pre-I/O | No backend escape; unsupported surfaces absent |
