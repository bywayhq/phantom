...
|---|---|---|---|---|---|---|---|
| **Phantom current** | Immutable routed client; no session/pool | Exact H1/H2/H3 GET; ordered origin and CONNECT fields; streaming body | Public typed TLS/H2/H3/QUIC profiles; H3 TLS is distinct and caller-supplied pending a retained browser fixture | Additive trust roots; direct TCP/UDP or plaintext HTTP CONNECT for H1/H2; no public DNS/cookies/client hints yet | Response body and bounded SSE decoder; no upload, WS, or reconnect policy | Stable facade/proxy/SSE error kinds; completion shuts down; early body drop cancels/resets; unsupported proxy/H3 pairing fails pre-I/O | No backend escape; unsupported surfaces absent |
