...
- Several layers, one browser. A [profile](reference/glossary.md#profile)
  can carry the TCP socket options, TLS ClientHello, HTTP/2 SETTINGS and
...
- Evidence you can check. Most browser recipes come from traffic recorded
  ...
  and [Validation](explanation/validation.md) says so.
- No silent fallback. A request for HTTP/3 never retries over HTTP/2, and
  a failed proxy never falls back to a direct connection. Phantom uses the
  protocol and [route](reference/glossary.md#route) you chose or returns a
  typed error, so the fingerprint never changes behind your back.
- Order is kept. Header fields, duplicates, and trailers go out in the
  order you add them, and SETTINGS and pseudo-header fields in the order the
  profile lists them.
- Bounded state owned by the client. Connection pools, cookies, Alt-Svc
  entries, and TLS session caches belong to one `Client`, each with a size
  limit. Nothing is global to the process.
- Safe Rust. The workspace forbids `unsafe` code, except in one private,
  documented module that calls BoringSSL's QUIC API.
...
- You need JavaScript to run. Phantom does not execute scripts, render
  pages, or produce canvas, font, or WebRTC signals. Use a browser automation
...
- You need many browser versions or operating systems. Phantom retires a
  browser version when it adds the next one. Tools that ship dozens of
  targets are listed [below](#compared-with-other-clients).
...
| [curl-impersonate](https://github.com/lexiforest/curl-impersonate) | C (a curl fork) | TLS, HTTP/2, HTTP/3 | 40+ Chrome, Edge, Safari, Firefox, and Tor targets |
| [curl_cffi](https://github.com/lexiforest/curl_cffi) | Python (bindings to curl-impersonate) | TLS, HTTP/2, HTTP/3 | 37 preset fingerprints in the open-source release |
| [wreq](https://github.com/0x676e67/wreq) | Rust | TLS, HTTP/2 | 100+ emulation profiles, kept in the separate `wreq-util` crate |
...
What sets Phantom apart, stated as facts rather than rankings:
...
- It ships far fewer browser builds than curl-impersonate, curl_cffi, or
  wreq, and it is not on crates.io, while wreq is.
