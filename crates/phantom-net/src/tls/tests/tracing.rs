...
use super::{TestResult, TlsConnector, TlsErrorKind};
...
fn successful_connector_build_records_outcome_without_error_kind() -> TestResult<()> {
    ...
    dispatcher::with_default(&dispatch, || {
        TlsConnector::new_with_roots(&v152_macos_tls(), std::iter::empty::<&[u8]>())
    })?;
...
async fn failed_handshake_records_static_error_kind() -> TestResult<()> {
    let connector = TlsConnector::new_with_roots(&v152_macos_tls(), std::iter::empty::<&[u8]>())?;
