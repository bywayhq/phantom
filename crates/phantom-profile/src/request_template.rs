...
fn validate_fields(
    ...
    for (index, template) in fields.iter().enumerate() {
        ...
        // A second credentials slot repeats the first one's name by design;
        // every other check above still applies to its own spelling.
        let repeated_slot = matches!(template, RequestField::ProxyAuthorization { .. })
